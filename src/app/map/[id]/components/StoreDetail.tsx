...
export default function StoreDetail({ store }: { store: Store | undefined }) {
  ...
  return (
    <div>
      <Header
        title={store?.name || "알 수 없음"}
        itemCount={Object.keys(cart).length}
      />

      <div className="-mx-4">
        <Image
          src={store?.image || "/example.png"}
          alt={store?.name || "알 수 없음"}
          width={600}
          height={300}
      ...
      <div className="flex gap-2">
        ...
        />
        <div className="mt-2">
          <h1 className="text-[18px] font-bold">{store?.name}</h1>
